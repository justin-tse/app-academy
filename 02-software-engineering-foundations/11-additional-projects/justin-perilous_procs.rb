...
def proctition(arr, &prc)
    begins = arr.select { |el| prc.call(el) }
    begins + (arr - begins)
end
...
def proc_suffix(sent, hash)
    words = sent.split
    suffix = Array.new(words.length, "")
    hash.each do |k, v|  
        words.each_with_index { |word, i| suffix[i] += v if k.call(word) }
    end
...
def proctition_platinum(arr, *procs)
    ...
    procs.each_with_index do |proc, i|
        arr.each do |word|
            if proc.call(word) && !hash.values.flatten.include?(word)
                hash[i + 1] << word
            end
        end
    end
...
def procipher(sent, hash)
    words = sent.split
    new_words = words.map { |word| word }
    hash.each do |k, v|
        (0...words.length).each do |i|
            new_words[i] = v.call(new_words[i]) if k.call(words[i])
        end
    end
    new_words.join(" ")
end
...
def picky_procipher(sent, hash)
    ...
    hash.each do |k, v|
        (0...words.length).each do |i|
             if k.call(words[i]) && count[i] == 0
                new_words[i] = v.call(new_words[i]) 
                count[i] += 1
             end
        end
    end
